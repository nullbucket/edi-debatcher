...
public class EdiValidatorDefault implements EdiValidator {
	...
	public boolean validate(long batchId, X12_ELEMENT elementName, String data, String compareWithData) throws Exception {
		...
		case ISA06:
			 ...
			 if(senders.length > 0 && !Arrays.asList(senders).contains(data)) {
				logError(batchId, TA1_ERROR_ISA06, ERROR.TYPE_TA1, "Invalid Interchange Sender ID(ISA06) " + data);
			}
			break;
		case ISA07:
			// Why is this hard-coded to ZZ?
			if (!equals("ZZ", data)) {
				logError(batchId, TA1_ERROR_ISA07, ERROR.TYPE_TA1, "Invalid Interchange Receiver ID(ISA07) Qualifier " + data); // ZZ Mutually Defined
			}
			break;
			
		case ISA08:
			 String[] receivers = config.getValidReceivers();
			 if(receivers.length > 0 && !Arrays.asList(receivers).contains(data)) {
				 logError(batchId, TA1_ERROR_ISA08, ERROR.TYPE_TA1, "Invalid Interchange Receiver ID(ISA08) " + data);
			 }	 
			break;
		case ISA11:
		...
		case ISA12:
			...
			break;
		case ISA13:
			if (StringUtils.isAllBlank(data) || data.length() != 9 || !isNumeric(data)) {
				logError(batchId, TA1_ERROR_ISA13, ERROR.TYPE_TA1, "Invalid ISA13 Interchange control number data " + data);
			}

			if (isa13List.contains(data)) {
				logError(batchId, TA1_ERROR_ISA13_DUP, ERROR.TYPE_TA1, "Duplicate ISA13 Interchange control number " + data);
			}
			isa13List.add(data);
		...
		case ISAEnd:
			if (!Arrays.asList(validISAEnd).contains(data)) {
				logError(batchId, TA1_ERROR_ISAEnd, ERROR.TYPE_TA1, "The Segment Terminator is invalid - ISA segment must be a fixed length " + data);
			}
			break;
		case IEA01:
			if (StringUtils.isAllBlank(data)) {
				logError(batchId, TA1_ERROR_IEA_MISSING, ERROR.TYPE_TA1, "Improper (Premature) end-of-file (Transmission) " + data);
			} else if (!equals(data, compareWithData)) {
				logError(batchId, TA1_ERROR_IEA01, ERROR.TYPE_TA1, "Invalid Number of Included Group value(IEA01) " + data);
			}
			break;
		case IEA02:
			break;
		case DATA_SEPARATOR:
			if (!Arrays.asList(validDataElementSeparator).contains(data)) {
				logError(batchId, TA1_ERROR_DATA_SEPARATOR, ERROR.TYPE_TA1, "Invalid data element separator - ISA segment must be a fixed length " + data);
			}
			break;
		case GS:
			if (StringUtils.isAllBlank(data)) {
		...
		case GS06:
			if (!equals(data, compareWithData)) {
				logError(batchId, AK9_999_ERROR_GS06, ERROR.TYPE_999, "Group Control Number in the Functional Group Header and trailer Do Not Agree " + data);
			}
			break;
		case GS08:
		...
		case GE01:
			if (!equals(data, compareWithData)) {
				logError(batchId, AK9_999_ERROR_GE01, ERROR.TYPE_999, "Number of Included Transaction Sets Does Not Match Actual Count " + data);
			}
			break;
		case ST01:
			if (StringUtils.isAllBlank(data) || !"837".equals(data)) {
				logError(batchId, IK5_999_ERROR_ST01, ERROR.TYPE_999, "Transaction Set Not Supported " + data);
			}
			break;
		case ST02:
			if (!equals(data, compareWithData)) {
				logError(batchId, IK5_999_ERROR_ST02, ERROR.TYPE_999, "Transaction Set Control Number in Header and Trailer Do Not Match " + data);
			}
			break;

		// TODO: This is domain-specific to 837 claims
		case ST03:
			if ((CLAIM_TYPE.PRO.name().equals(compareWithData) && !Arrays.asList(validSt03ForP).contains(data))
				...
				logError(batchId, IK5_999_ERROR_ST03, ERROR.TYPE_999, "Invalid Transaction Set Implementation Convention Reference " + data);
			}
			break;
		case SE:
			if (StringUtils.isAllBlank(data)) {
		...
		case SE01:
			if (!equals(data, compareWithData)) {
				logError(batchId, IK5_999_ERROR_SE01, ERROR.TYPE_999, "Number of Included Segments Does Not Match Actual Count " + data);
			}
			break;
		default:
			break;
