...
public class DefaultValidator implements Validator {
	...
	public boolean validate(long batchId, X12_ELEMENT elementName, String actual, String expected) throws Exception {
		...
		case ST03:
			if ((CLAIM_TYPE.PRO.name().equals(expected) && !Arrays.asList(validSt03ForP).contains(trimmed))
					|| (CLAIM_TYPE.INS.name().equals(expected) && !Arrays.asList(validSt03ForI).contains(trimmed))
					|| !CLAIM_TYPE.PRO.name().equals(expected)
							&& !CLAIM_TYPE.INS.name().equals(expected)) {
